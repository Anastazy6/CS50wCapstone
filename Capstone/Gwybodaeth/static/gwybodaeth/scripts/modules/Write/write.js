...
export const Write = function () {
  ...
  const _startView = () => {
    ...
    let methods = {
      ...
      retry: _retryItem,
      resolve: target => {
        WriteUtilities.resolve(_resolutionMethods(), target);
      }
    };
  ...
  const _submitAnswerWrapper = () => {
    ...
    if (!input) {
      return false;
    } // Prevent accidentally sending empty input.
    View.Write.hide();
  ...
  const _showView = view => {
    const views = [View.Feedback, View.Write, View.Summary];
  ...
  const _showRoundsSummary = () => {
    const data = Memory.countIncorrect() > 0 ? Memory.getLastRound() : Memory.getAllRounds();
