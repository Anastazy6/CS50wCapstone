...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (route[0] === 'create-set') {
    import("./modules/Create/create.mjs").then(createModule => {
      createModule.Create.run();
    });
  }
  if (route[2] === 'flashcards') {
    import("./modules/Flashcards/flashcards.mjs").then(flashcardsModule => {
      Load.justTerms(flashcardsModule.Flashcards.loadFlashcards);
    });
  }
  if (route[2] === 'learn') {
    import("./modules/Learn/learn.mjs").then(learnModule => {
      Load.justTerms(learnModule.Learn.loadItems);
    });
  }
  if (route[2] === 'write') {
    import("./modules/Write/write.mjs").then(writeModule => {
